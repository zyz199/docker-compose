...
docker build -f Dockerfile --build-arg JAVA_OPTS="-XX:+UseG1GC -Xms64m -Xmx64m -Xmn16m -XX:MetaspaceSize=100m -XX:MaxMetaspaceSize=100m -XX:MaxGCPauseMillis=200 -XX:ParallelGCThreads=8 -Ddefault.client.encoding=UTF-8 -Dfile.encoding=UTF-8 -Duser.language=Zh -Duser.region=CN" --build-arg APP_NAME="demo" --build-arg APP_PORT="80" -t "registry.cn-hangzhou.aliyuncs.com/zhangyuezong/demo:dev" . --no-cache

# 推送镜像
docker push registry.cn-hangzhou.aliyuncs.com/zhangyuezong/demo:dev

# 拉取镜像
docker pull registry.cn-hangzhou.aliyuncs.com/zhangyuezong/demo:dev

# 运行
docker run -d -p 80:80 -v /home/zhangyuezong/demo.log:/home/demo.log --name demo registry.cn-hangzhou.aliyuncs.com/zhangyuezong/demo:dev
